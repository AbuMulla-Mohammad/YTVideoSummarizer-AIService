...
from app.utils import extract_video_id,get_video_transcript,convert_transcript_to_text,summarize_format_text
from app.models.youtube_models import VideoIDResponse, TranscriptRequest, TranscriptTextResponse, SummarizeFormatTranscriptRequest
...
async def convert_transcript_text_endpoint(request: TranscriptRequest,response_model=TranscriptTextResponse):
...
async def summarize_format_transcript(request:SummarizeFormatTranscriptRequest):
    ...
    result=summarize_format_text(text=text["text"],model=request.model,prompt=prompt_template)
    if "error" in result:
